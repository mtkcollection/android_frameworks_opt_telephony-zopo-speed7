...
public final class GsmCallTrackerHelper {
    ...
    public void CallIndicationProcess(AsyncResult ar) {
        ...
        log("PROPERTY_TERMINAL_BASED_CALL_WAITING_MODE = "
            + SystemProperties.get(PROPERTY_TERMINAL_BASED_CALL_WAITING_MODE)
            + ",ForgroundCall State=" + mTracker.mForegroundCall.getState()
            + ",BackgroundCall State=" + mTracker.mBackgroundCall.getState());
        if (SystemProperties.get(PROPERTY_TERMINAL_BASED_CALL_WAITING_MODE).equals(
               TERMINAL_BASED_CALL_WAITING_ENABLED_OFF) == true
            && ((mTracker.mForegroundCall != null &&
                 mTracker.mForegroundCall.getState() == GsmCall.State.ACTIVE)
                || (mTracker.mBackgroundCall != null &&
                    mTracker.mBackgroundCall.getState() == GsmCall.State.HOLDING))) {
            log("PROPERTY_TERMINAL_BASED_CALL_WAITING_MODE = "
